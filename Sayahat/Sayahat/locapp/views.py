...
def DestinationView(request):
    ...
    popular_dest=DestinationMetaDetails.objects.filter(admin_approved=True,main_page_content=True,place_rating__in=[4,5]).order_by('?')[0:8]
    fm = DestinationForm()
    obj = DestinationCityDetails.objects.filter(admin_approved=True,main_page_content=True, city_rating__in=[4,5])[0:8]
    obj1 = DestinationCityDetails.objects.filter(admin_approved=True,main_page_content=True,).order_by('?')[0:8]
    all_obj=DestinationCityDetails.objects.filter(admin_approved=True,main_page_content=True)
